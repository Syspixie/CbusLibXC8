...
#include "moduledefs.h"
    ...
    typedef union {
        struct {
            // Info
            uint8_t rxMsgCount;     // # messages received
            uint8_t rxFifoUsed;     // Current # receive FIFO entries used
            uint8_t rxFifoMaxUsed;  // Maximum # receive FIFO entries used
            uint8_t txMsgCount;     // # messages transmitted
            uint8_t txFifoUsed;     // Current # transmit FIFO entries used
            uint8_t txFifoMaxUsed;  // Maximum # transmit FIFO entries used
            uint8_t txPrioHike;     // # times transmit message priority raised
            // Errors
            uint8_t rxHwFifoFull;   // # hardware FIFO full errors
            uint8_t rxSwFifoFull;   // # receive software FIFO full errors
            uint8_t txSwFifoFull;   // # transmit software FIFO full errors
            uint8_t txTmoErr;       // # transmit message timeouts
            uint8_t rxMsgDiscarded; // # receive messages discarded to avoid errors
            uint8_t txBusErr;       // # bus errors
            uint8_t busInvMsg;      // # invalid message errors
        };
        uint8_t bytes[16];
    } stats_t;
